...
type Product = { id: number; name: string; packageName: string; purchasePrice: number; defaultSalePrice: number; stock: number; lowStockAt: number; supplier: string; stockNote: string };
type Sale = { id: number; productId: number; productName: string; customerName: string; customerContact: string; quantity: number; purchasePrice: number; salePrice: number; paidAmount: number; paymentMethod: string; paymentStatus: "paid" | "partial" | "unpaid"; saleDate: string; expiryDate: string; staffName: string; note: string };
    ...
    <main className="min-h-screen bg-[var(--background)] pb-24 text-[var(--foreground)] md:pb-10">
      ...
      <div className="mx-auto max-w-6xl px-4 pt-5 md:px-8 md:pt-8">
        ...
        <Tabs defaultValue="sales" className="gap-5">
          ...
          <TabsContent value="sales"><SalesList sales={filteredSales} products={data?.products ?? []} search={search} setSearch={setSearch} isAdmin={data?.user.role === "admin"} saving={saving} onSubmit={submit} /></TabsContent>
          <TabsContent value="stock"><StockList products={data?.products ?? []} isAdmin={data?.user.role === "admin"} saving={saving} onSubmit={submit} /></TabsContent>
...
function SalesList({ sales, products, search, setSearch, isAdmin, saving, onSubmit }: { sales: Sale[]; products: Product[]; search: string; setSearch: (s: string) => void; isAdmin: boolean; saving: boolean; onSubmit: Submit }) {
  return <section><div className="mb-4 flex items-center justify-between gap-3"><h3 className="font-semibold text-white">နောက်ဆုံးအရောင်းများ</h3><div className="relative w-48 md:w-64"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-500" /><Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Customer ရှာရန်" className="border-white/10 bg-white/5 pl-9 text-white" /></div></div><div className="space-y-3">{sales.length ? sales.map(s => <SaleCard key={s.id} sale={s} product={products.find(p => p.id === s.productId)} isAdmin={isAdmin} saving={saving} onSubmit={onSubmit} />) : <Empty text="အရောင်းစာရင်း မရှိသေးပါ" />}</div></section>;
}
function SaleCard({ sale: s, product, isAdmin, saving, onSubmit }: { sale: Sale; product?: Product; isAdmin: boolean; saving: boolean; onSubmit: Submit }) {
  const remaining = s.salePrice * s.quantity - s.paidAmount;
  return <article className="rounded-2xl border border-white/8 bg-[#101a2b] p-4 transition hover:border-white/15"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="mb-1 flex items-center gap-2"><h4 className="truncate font-semibold text-white">{s.customerName}</h4><Status status={s.paymentStatus} /></div><p className="truncate text-sm text-slate-400">{s.productName} · {s.quantity} ခု</p></div><p className="shrink-0 font-bold text-emerald-300">{money(s.salePrice * s.quantity)}</p></div><div className="mt-4 grid grid-cols-2 gap-2 border-t border-white/7 pt-3 text-xs text-slate-500 md:grid-cols-4"><span>ဝယ်ရက် · {s.saleDate}</span><span>ကုန်ရက် · {s.expiryDate}</span><span>{s.paymentMethod}{remaining > 0 ? ` · ${money(remaining)} ကျန်` : ""}</span><span className="md:text-right">{s.staffName}</span></div>{isAdmin && <div className="mt-3 flex items-center justify-between gap-2"><p className="text-xs text-cyan-300/80">အမြတ် {money((s.salePrice - s.purchasePrice) * s.quantity)}</p><EditSaleDialog sale={s} product={product} saving={saving} onSubmit={onSubmit} /></div>}</article>;
}
function StockList({ products, isAdmin, saving, onSubmit }: { products: Product[]; isAdmin: boolean; saving: boolean; onSubmit: Submit }) {
  return <section><h3 className="mb-4 font-semibold text-white">Product & Stock</h3><div className="grid gap-3 md:grid-cols-2">{products.length ? products.map(p => <article key={p.id} className="rounded-2xl border border-white/8 bg-[#101a2b] p-4"><div className="flex items-start justify-between"><div><h4 className="font-semibold text-white">{p.name}</h4><p className="text-sm text-slate-500">{p.packageName || "Package မသတ်မှတ်ထား"}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${p.stock <= p.lowStockAt ? "bg-amber-400/15 text-amber-300" : "bg-emerald-400/12 text-emerald-300"}`}>{p.stock} ခု</span></div><div className="mt-5 grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs text-slate-500">ရောင်းဈေး</p><p className="mt-1 text-slate-200">{money(p.defaultSalePrice)}</p></div>{isAdmin && <div><p className="text-xs text-slate-500">ဝယ်ဈေး</p><p className="mt-1 text-slate-200">{money(p.purchasePrice)}</p></div>}</div>{p.stockNote && <p className="mt-3 rounded-lg bg-red-400/10 p-2 text-xs leading-5 text-red-200">{p.stockNote}</p>}<div className="mt-4 flex items-center justify-between gap-2">{p.stock <= p.lowStockAt ? <p className="flex items-center gap-1.5 text-xs text-amber-300"><AlertTriangle className="size-3.5" />Stock ပြန်ဖြည့်ရန်လိုပါပြီ</p> : <span />}{isAdmin && <EditProductDialog product={p} saving={saving} onSubmit={onSubmit} />}</div></article>) : <Empty text="Product မထည့်ရသေးပါ" />}</div></section>;
}
...
function ProductDialog({ open, setOpen, onSubmit, saving, isAdmin }: { open: boolean; setOpen: (v: boolean) => void; onSubmit: Submit; saving: boolean; isAdmin?: boolean }) { if (!isAdmin) return null; return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" className="border-white/10 bg-white/5 text-white hover:bg-white/10"><PackagePlus /> <span className="hidden md:inline">Product ထည့်ရန်</span></Button></DialogTrigger><DialogContent className="max-h-[90vh] overflow-y-auto border-white/10 bg-[#101a2b] text-white"><DialogHeader><DialogTitle>Product အသစ်ထည့်ရန်</DialogTitle><DialogDescription>ဈေးနှုန်းနဲ့ Stock ကို တစ်ခါတည်းသတ်မှတ်ပါ။</DialogDescription></DialogHeader><form onSubmit={e => void onSubmit(e, setOpen)} className="form-grid"><input type="hidden" name="action" value="add-product" /><Field label="Product Name"><Input name="name" placeholder="ဥပမာ Gemini Pro" required /></Field><Field label="Package"><Input name="packageName" placeholder="ဥပမာ 1 Month" /></Field><div className="grid grid-cols-2 gap-3"><Field label="ဝယ်ဈေး"><Input name="purchasePrice" type="number" inputMode="numeric" min="0" required /></Field><Field label="ရောင်းဈေး"><Input name="defaultSalePrice" type="number" inputMode="numeric" min="0" required /></Field></div><div className="grid grid-cols-2 gap-3"><Field label="Stock"><Input name="stock" type="number" inputMode="numeric" min="0" required /></Field><Field label="Low Stock Alert"><Input name="lowStockAt" type="number" defaultValue="5" min="0" /></Field></div><Field label="Supplier"><Input name="supplier" placeholder="Supplier name / contact" /></Field><Field label="Stock မှတ်ချက်"><Input name="stockNote" placeholder="ဥပမာ Product Error / ပျက်ဆုံး" /></Field><Button disabled={saving} className="mt-2 h-11 bg-emerald-400 text-slate-950 hover:bg-emerald-300">{saving ? "သိမ်းနေပါတယ်…" : "Product သိမ်းမယ်"}</Button></form></DialogContent></Dialog>; }

function EditProductDialog({ product, saving, onSubmit }: { product: Product; saving: boolean; onSubmit: Submit }) {
  const [open, setOpen] = useState(false);
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button size="sm" variant="outline" className="border-white/10 bg-white/5 text-white">ပြင်ရန်</Button></DialogTrigger><DialogContent className="max-h-[92vh] overflow-y-auto border-white/10 bg-[#101a2b] text-white"><DialogHeader><DialogTitle>Product & Stock ပြင်ရန်</DialogTitle><DialogDescription>ပျက်သွားသော Stock ကို လက်ရှိအရေအတွက်မှ လျှော့ပြီး မှတ်ချက်ရေးနိုင်ပါတယ်။ ငွေစာရင်းကို သီးခြားပြင်ပါ။</DialogDescription></DialogHeader><form onSubmit={e => void onSubmit(e, setOpen)} className="form-grid"><input type="hidden" name="action" value="edit-product" /><input type="hidden" name="id" value={product.id} /><Field label="Product Name"><Input name="name" defaultValue={product.name} required /></Field><Field label="Package"><Input name="packageName" defaultValue={product.packageName} /></Field><div className="grid grid-cols-2 gap-3"><Field label="ဝယ်ဈေး"><Input name="purchasePrice" type="number" min="0" step="1" defaultValue={product.purchasePrice} required /></Field><Field label="ရောင်းဈေး"><Input name="defaultSalePrice" type="number" min="0" step="1" defaultValue={product.defaultSalePrice} required /></Field></div><div className="grid grid-cols-2 gap-3"><Field label="လက်ရှိ Stock"><Input name="stock" type="number" min="0" step="1" defaultValue={product.stock} required /></Field><Field label="Low Stock Alert"><Input name="lowStockAt" type="number" min="0" step="1" defaultValue={product.lowStockAt} required /></Field></div><Field label="Supplier"><Input name="supplier" defaultValue={product.supplier} /></Field><Field label="Stock မှတ်ချက်"><Input name="stockNote" defaultValue={product.stockNote} placeholder="ဥပမာ Product Error — 2 ခုဆုံးရှုံး" /></Field><Button disabled={saving} className="h-11 bg-emerald-400 text-slate-950">{saving ? "သိမ်းနေပါတယ်…" : "ပြင်ပြီးသိမ်းမယ်"}</Button></form></DialogContent></Dialog>;
}

function EditSaleDialog({ sale, product, saving, onSubmit }: { sale: Sale; product?: Product; saving: boolean; onSubmit: Submit }) {
  const [open, setOpen] = useState(false);
  const deleteSubmit = (e: FormEvent<HTMLFormElement>) => { if (!window.confirm("ဒီအရောင်းစာရင်းကို ဖျက်မှာ သေချာပါသလား?")) { e.preventDefault(); return; } void onSubmit(e, setOpen); };
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button size="sm" variant="outline" className="border-white/10 bg-white/5 text-white">ပြင်ရန်</Button></DialogTrigger><DialogContent className="max-h-[92vh] overflow-y-auto border-white/10 bg-[#101a2b] text-white"><DialogHeader><DialogTitle>အရောင်းစာရင်းပြင်ရန်</DialogTitle><DialogDescription>{sale.productName} — Quantity ပြောင်းလျှင် Stock ကို အလိုအလျောက်ညှိပေးပါတယ်။</DialogDescription></DialogHeader><form onSubmit={e => void onSubmit(e, setOpen)} className="form-grid"><input type="hidden" name="action" value="edit-sale" /><input type="hidden" name="id" value={sale.id} /><Field label="Customer Name"><Input name="customerName" defaultValue={sale.customerName} required /></Field><Field label="ဖုန်း / Telegram"><Input name="customerContact" defaultValue={sale.customerContact} /></Field><div className="grid grid-cols-2 gap-3"><Field label="အရေအတွက်"><Input name="quantity" type="number" min="1" max={(product?.stock ?? 0) + sale.quantity} defaultValue={sale.quantity} required /></Field><Field label="ရောင်းဈေး (တစ်ခု)"><Input name="salePrice" type="number" min="0" step="1" defaultValue={sale.salePrice} required /></Field></div><div className="grid grid-cols-2 gap-3"><Field label="ရောင်းသည့်ရက်"><Input name="saleDate" type="date" defaultValue={sale.saleDate} required /></Field><Field label="Expired Date"><Input name="expiryDate" type="date" defaultValue={sale.expiryDate} required /></Field></div><div className="grid grid-cols-2 gap-3"><Field label="ရရှိငွေ"><Input name="paidAmount" type="number" min="0" step="1" defaultValue={sale.paidAmount} required /></Field><Field label="ငွေပေးချေမှု"><select name="paymentMethod" defaultValue={sale.paymentMethod} className="h-11 rounded-xl border border-white/10 bg-[#101a2b] px-3">{["KPay", "Wave", "Bank", "Cash", "USDT"].map(method => <option key={method}>{method}</option>)}</select></Field></div><Field label="မှတ်ချက်"><Input name="note" defaultValue={sale.note} /></Field><Button disabled={saving} className="h-11 bg-emerald-400 text-slate-950">{saving ? "သိမ်းနေပါတယ်…" : "ပြင်ပြီးသိမ်းမယ်"}</Button></form><form onSubmit={deleteSubmit} className="mt-2 space-y-3 border-t border-white/10 pt-4"><input type="hidden" name="action" value="delete-sale" /><input type="hidden" name="id" value={sale.id} /><Field label="အရောင်းဖျက်ပြီးနောက်"><select name="restoreStock" defaultValue="yes" className="h-11 rounded-xl border border-white/10 bg-[#101a2b] px-3"><option value="yes">Stock ကို ပြန်တိုးမယ်</option><option value="no">Stock မပြန်တိုးဘူး (ပျက်/ဆုံးရှုံး)</option></select></Field><Button disabled={saving} variant="outline" className="w-full border-red-400/30 bg-red-400/10 text-red-200 hover:bg-red-400/20">အရောင်းစာရင်းဖျက်မယ်</Button></form></DialogContent></Dialog>;
}
