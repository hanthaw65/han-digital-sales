...
export function Finance({ entries, summary, isAdmin, saving, onSubmit }: { entries: FinanceEntry[]; summary: FinanceSummary; isAdmin: boolean; saving: boolean; onSubmit: Submit }) {
  return <section className="space-y-5">
    ...
    <p className="text-xs leading-6 text-slate-400">Product အသစ်ထည့်တဲ့အခါ ဝယ်ဈေး × Stock ကို အပြည့်ပေးချေထားတဲ့ ဝယ်ငွေအဖြစ် အလိုအလျောက်မှတ်ပါတယ်။ အရင်ဝယ်ထားတာတွေကို “Stock ဝယ်ငွေထည့်ရန်” မှာ တစ်ကြိမ်သာဖြည့်ပါ။ ဒီနေရာက ငွေစာရင်းထည့်ခြင်းသည် Stock အရေအတွက်ကို မပြောင်းပါ။</p>
    <div className="space-y-3">{entries.map(entry => <article key={entry.id} className="rounded-2xl border border-white/10 bg-[#101a2b] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs text-slate-400">{entry.kind === "capital" ? "ထည့်ဝင်အရင်း" : "Stock ဝယ်ငွေ"}{entry.automatic ? " · Auto" : ""}</p><h4 className="mt-1 break-words font-semibold text-white">{entry.label}</h4></div><p className={`shrink-0 font-bold ${entry.kind === "capital" ? "text-emerald-300" : "text-amber-300"}`}>{money(entry.amount)}</p></div><p className="mt-3 text-xs text-slate-400">{entry.entryDate} · {entry.paymentMethod}</p>{entry.note && <p className="mt-2 break-words text-xs leading-5 text-slate-500">{entry.note}</p>}{isAdmin && !entry.automatic && <div className="mt-3"><EntryDialog kind={entry.kind} entry={entry} saving={saving} onSubmit={onSubmit} /></div>}</article>)}{!entries.length && <p className="py-8 text-center text-slate-400">ငွေစာရင်း မရှိသေးပါ</p>}</div>
